...
export function Header() {
  ...
  return (
    <>
      {/* TOP CONTACT BAR */}
      <div className="bg-[#00afea] text-white text-xs hidden md:block relative z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-9">
          ...
          <div className="flex items-center gap-4">
            <a
              ...
              aria-label="Facebook CHÂU Á THÀNH CÔNG"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              aria-label="TikTok CHÂU Á THÀNH CÔNG"
              className="hover:text-red-100 transition-colors"
            >
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-18 lg:h-20">
            <Link href="/" aria-label="Trang chủ CHÂU Á THÀNH CÔNG" className="flex items-center gap-3 shrink-0">
              <img src="/images/thanh-cong-45-logo.png" alt="CHÂU Á THÀNH CÔNG Logo" className="h-10 lg:h-12 w-auto" loading="eager" decoding="async" width="160" height="48" />
            </Link>
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`px-4 py-2.5 rounded-lg text-sm font-bold uppercase tracking-wide transition-all duration-200 ${pathname === item.path
                    ? 'text-[#00afea] bg-[rgba(0, 175, 234,0.06)]'
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              <button
                ...
                className="bg-[#00afea] text-white px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-[#d41111] transition-all hover:shadow-lg hover:shadow-red-200 duration-300"
              >
                Ứng tuyển ngay
              </button>
            </div>
        ...
        {mobileMenuOpen && (
          <div id="mobile-navigation" className="md:hidden bg-white border-t border-slate-100 px-4 py-4 flex flex-col gap-1 shadow-lg">
            {navItems.map((item) => (
              <Link
                ...
                className={`px-4 py-3 rounded-lg text-sm font-bold uppercase tracking-wide ${pathname === item.path
                  ? 'text-[#00afea] bg-[rgba(0, 175, 234,0.05)]'
                  : 'text-[#475569]'
                  }`}
              >
                {item.label}
            ...
            <button
              type="button"
              onClick={() => router.push('/tuyen-dung/ung-tuyen')}
              className="mt-2 bg-[#00afea] text-white px-5 py-3 rounded-lg text-sm font-semibold text-center"
            >
              Ứng tuyển ngay
            </button>
          </div>
