...
import { fallbackNews, seededNews, categoryMap, extractTextFromRichText, type NewsItem } from './news-data'
...
export default async function NewsPage() {
  ...
  try {
    ...
    if (docs && docs.length > 0) {
      const payloadNews = docs.map((doc: any) => {
      ...
      news = [
        ...
        ...payloadNews.filter(
          (item) =>
            !seededNews.some(
              (seededItem) =>
                seededItem.title.trim().toLowerCase() === item.title.trim().toLowerCase(),
            ),
        ),
      ]
    }
